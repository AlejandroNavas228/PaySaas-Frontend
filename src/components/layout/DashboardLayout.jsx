...
export default function DashboardLayout({ children }) {
  ...
  const correoAdmin = "alejandronavas228@gmail.com".trim().toLowerCase(); 
  
  if (comercio?.email?.trim().toLowerCase() === correoAdmin) {
    menuItems.push({ 
      name: 'Panel Admin', 
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      {/* SIDEBAR CON NUEVO COLOR (slate-950) Y BORDES */}
      <aside className={`fixed inset-y-0 left-0 w-64 bg-slate-950 text-white z-50 transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
            {menuItems.map((item) => (
              <button
                ...
                // BOTONES CON ROUNDED-2XL Y NUEVAS SOMBRAS
                className={`w-full flex items-center justify-between px-4 py-3.5 rounded-2xl transition-all group
                  ${location.pathname === item.path ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-white/5 hover:text-white'}
            ...
            <div className="pt-4 mt-4 border-t border-white/5">
              <button onClick={() => navigate('/planes')} className="w-full flex items-center gap-3 px-4 py-3.5 text-sm font-bold text-amber-400 hover:bg-amber-400/10 rounded-2xl transition-all">
          ...
          <div className="p-4 border-t border-white/5">
            <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3.5 text-sm font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-2xl transition-all">
        ...
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in zoom-in-95 duration-200">
